...
inline_tugma3 = InlineKeyboardMarkup(
    inline_keyboard=[[InlineKeyboardButton(text="🎬 𝗞𝗶𝗻𝗼 𝗾𝗼'𝘀𝗵𝗶𝘀𝗵",url="https://javlon6bek.pythonanywhere.com/admin/kinolar/moviesmodel/")],
...
async def inlinebutton(callback: types.CallbackQuery):
    await callback.answer(text=f"📲  Kanalga kirib  📥 Yuklash\n            tugmasini bosing\n\nYoki 🔍Kod yozib yuboring \n    ♻️Kod bu =  biror son",show_alert=True)
...
async def hastart(message: Message):
    ...
    if id not in user_ids:
        ...
        await bot.send_message(ADMIN_ID, f"*✅ Yangi Foydanuvchi\n\n👤 Ismi : {odi}\n🆔 IDsi : __{id}__\n🤖 Jami: {len(user_ids)} Ta foydalanuvchi*",
...
async def admin(message: types.Message):
    ...
    await bot.send_sticker(id,sticker=stiker)
    await message.answer(f"📊 𝗕𝗼𝘁 𝗦𝘁𝗮𝘁𝗶𝘀𝗸𝗮\n👾 @AJyUzBot\n\n\n✅  𝗙𝗮𝗼𝗹 - {len(m1)}\n❌ 𝗡𝗼𝗳𝗮𝗼𝗹 - {len(m2)}\n👥 𝗝𝗮𝗺𝗶  - {len(user_ids)}\n\n\n⚙ 𝗬𝗮𝗿𝗮𝘁𝗶𝗹𝗱𝗶 8.05.2024")
...
async def kino(message: types.Message):
    for i in r:
        if int(message.text) == int(i['kod']):
               async with ChatActionSender.upload_video(message.from_user.id, bot):
                await bot.send_sticker(message.from_user.id,sticker="CAACAgEAAxkBAAEMDtdmN860jf2dHnPGmTkdaz8Wd9X7HgACLQIAAqcjIUQ9QDDJ7YO0tjUE")
